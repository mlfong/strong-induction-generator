...
public class SIApplet extends JApplet
{
    ...
    public void init()
    {
        ...
        for (int i = 0; i < small; i++)
        {
            ...
            basisStepJLabels.add(new JLabel("" + small
                    + (bool ? " cent(s)" : " dollars(s)")));
            basisStepJLabels.add(new JLabel("" + big
                    + (bool ? " cent(s)" : " dollar(s).")));
        }// end for
        for (int i = 0; i < basisStepJLabels.size(); i = i + 4)
        {
            basisStepJLabels.get(i).setBounds(92, 61 + (i / 4) * 25, 16, 14);
            basisStepJLabels.get(i + 1).setBounds(133, 61 + (i / 4) * 25, 16,
                    14);
            basisStepJLabels.get(i + 2).setBounds(180, 61 + (i / 4) * 25, 110,
                    14);
            basisStepJLabels.get(i + 3).setBounds(310, 61 + (i / 4) * 25, 110,
                    14);
        ...
        {
            // set bounds
            basisStepJTextFields.get(i).setBounds(109, 58 + (i / 3) * 25, 23,
                    20);
            basisStepJTextFields.get(i + 1).setBounds(142, 58 + (i / 3) * 25,
                    23, 20);
            basisStepJTextFields.get(i + 2).setBounds(274, 58 + (i / 3) * 25,
                    23, 20);
            // make answers
        ...
        exitB = new JButton("Get new problem!");
        ...
        lblNewLabel_2 = new JLabel((bool ? "cents" : "dollars")
                + " so we can add a");
        ...
        lblNewLabel_3 = new JLabel((bool ? "cent stamp" : "dollar bill")
                + " to make");
        ...
        lblTheStatement = new JLabel(", the statement is true for all "
                + (bool ? "postage" : "change") + " greater than or equal to");
    ...
    public class AnswerButtonHandler implements ActionListener
    {
        ...
        {
            textField_1.setText("" + SIApplet.this.minBasis);
            textField_2.setText("j");
            textField_3.setText("k");
            textField_4.setText(""
                    + (SIApplet.this.minBasis + SIApplet.this.small - 1));
            textField_5.setText("k");
            textField_6.setText("k-" + (SIApplet.this.small - 1));
            textField_7.setText("" + SIApplet.this.small);
            ...
            textField_12.setText("" + SIApplet.this.minBasis);
            ...
            if (someNum == 1)
                JOptionPane
                        .showMessageDialog(null,
                                "Minimum postage/dollar amount is (n-1)(m-1) for the two given values.");
            else if (someNum == 2)
                JOptionPane.showMessageDialog(null,
                        "Number of base cases is the min(m,n).");
            else if (someNum == 3)
                JOptionPane.showMessageDialog(null,
                        "For the Ind. Step, we want a number in range of j.");
            else if (someNum == 4)
                JOptionPane.showMessageDialog(null,
                        "Don't forget to write out the whole conclusion.");
            else if (someNum == 5)
                JOptionPane
                        .showMessageDialog(null,
                                "The given two values, m and n, must be coprime to work!");
            else if (someNum == 6)
                JOptionPane.showMessageDialog(null,
    ...
    public class CalculateButtonHandler implements ActionListener
    {
        public void actionPerformed(ActionEvent e)
        {
            ...
            if (!SIApplet.isNum(minBasis2)
                    || Integer.parseInt(minBasis2) != SIApplet.this.minBasis)
            {
                numWrong++;
            ...
            if (!SIApplet.isNum(small)
                    || Integer.parseInt(small) != SIApplet.this.small)
            {
                numWrong++;
            ...
            String oneLess = "k-" + (SIApplet.this.small - 1);
            ...
            if (!SIApplet.isNum(minBasis)
                    || Integer.parseInt(minBasis) != SIApplet.this.minBasis)
            {
                numWrong++;
            ...
            if (!SIApplet.isNum(maxBasis)
                    || Integer.parseInt(maxBasis) != SIApplet.this.minBasis
                            + SIApplet.this.small - 1)
            {
                numWrong++;
    ...
    public void stop()
    {
        ...
        basisStepJTextFields = null;
        basisStepJLabels = null;
        basisStepAnswers = null;
        ...
        lblNewLabel = null;
        lblNewLabel_1 = null;
        lblInductiveStep = null;
        lblWeCanForm = null;
        lblNewLabel_2 = null;
        lblNewLabel_3 = null;
        lblNewLabel_4 = null;
        lblConclusion = null;
        lblWeHaveCompleted = null;
        lblAndThe = null;
        lblSoBy = null;
        lblTheStatement = null;
        lblNewLabel_5 = null;
